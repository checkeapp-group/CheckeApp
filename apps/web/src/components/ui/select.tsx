      ...
      <Select.Portal>
        <Select.Content className="z-50 rounded-md border border-input bg-popover text-popover-foreground shadow-md">
          <Select.Viewport className="p-1">
            {options.map((opt) => (
              <Select.Item
                className="relative flex cursor-pointer items-center rounded-sm px-8 py-2 text-foreground text-sm outline-none hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[state=checked]:bg-accent data-[state=checked]:text-accent-foreground data-[disabled]:opacity-50"
                key={opt.value}
                value={opt.value}
              >
