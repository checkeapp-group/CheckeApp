...
export default function BaseLayout({ children, showHeader = true }: LayoutProps) {
  const { t } = useI18n();
      ...
      <main className="flex-1">
        {isLoading ? (
          <div className="flex min-h-[300px] items-center justify-center">
            <p className="text-muted-foreground text-sm">{t('auth.checking')}</p>
          </div>
        ) : (
          <>
            {children}
            {/* Show TextInputForm only on homepage when authenticated */}
            {isAuthenticated && pathname === '/' && <TextInputForm />}
          </>
        )}
      </main>
      {/* Footer */}
